...
public:
	...
	bool operator==(const Iterator<T>& rhs) const; // needed for reverse() called by iter_swap()
...
{
	return !(this->node == rhs.node && this->pastTheEnd == rhs.pastTheEnd);
}
...
Iterator<T> Iterator<T>::operator++(int)
{
	...
	if (this->node->forward) {
		this->node = this->node->forward;
	}
	else {
...
{
	Iterator<T> tempPtr;
	tempPtr.node = this->first;
	if (!(this->first)) {
		tempPtr.pastTheEnd = true; // if list is empty (first = nullptr) then set pastTheEnd = true
	}
	return tempPtr;
}
...
{
	Iterator<T> tempPtr;
	tempPtr.node = this->last;
	tempPtr.pastTheEnd = true;
	return tempPtr;
}
...
void ourReverse(T begin, T end) {
	--end;
	auto tempPtr = end;
	// as we walk the nodes from both ends, begin == end when we reach the middle and
	// have an odd number of nodes, tempPtr trails end by one node and begin == tempPtr
	// when we reach the middle and have an even number of nodes
	while (begin != end && begin != tempPtr) {
		iter_swap(begin, end);
		++begin;
		tempPtr = end;
		--end;
